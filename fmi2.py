...
fmi2Type                 = c_int
fmi2Byte                 = c_char
fmi2RealArray            = POINTER(fmi2Real)
...
class _FMU2(_FMU):
    ...
    def __init__(self, **kwargs):
        ...
        self._fmi2Function('fmi2GetRealArrayEx',
                           ['component', 'vr', 'nvr', 'value'],
                           [fmi2Component, POINTER(fmi2ValueReference), c_size_t, POINTER(fmi2RealArray)]
        )
        ...
        self._fmi2Function('fmi2SetRealArrayEx',
                           ['component', 'vr', 'nvr', 'value'],
                           [fmi2Component, POINTER(fmi2ValueReference), c_size_t, POINTER(fmi2RealArray)]
        )
    ...
    def _fmi2Function(self, fname, argnames, argtypes, restype=fmi2Status):
        ...
        # import ctypes
        # import os
        # # print(os.path.abspath(self.dll))
        # my_dll = ctypes.CDLL("blossom_1.dll")
        # try:
        #     my_function = getattr(my_dll, 'fmi2GetRealArray')
        # except AttributeError:
        #     print(f"Function 'my_function_name' not found in blossom_1")

        # print("self.dll", self.dll, "fname", fname)
    ...
    def getReal(self, vr):
        ...
        print("vr变成：",vr,"len(vr)是：",len(vr))
        print("fmi2Real是：",fmi2Real)
        value = (fmi2Real * len(vr))()
        print("value是：",value)
        print("self.component是：",self.component)
    ...
    # 方案1
    def getRealArray(self, vr):
        vr = (fmi2ValueReference * len(vr))(*vr)
        value = (fmi2RealArray * len(vr))()
        self.fmi2GetRealArrayEx(self.component, vr, len(vr), value)
        return list(value)

    # 方案2
    # def getRealArray(self, vr, size):
    #     """
    #     Retrieve an array of Real values.
    #
    #     :param vr: Value references (list of indices for the array variable)
    ...
    def setRealArray(self, vr, value):
        """Set array of real values in the FMU.

        Parameters:
            vr (list): List of value references
            values (list): List of arrays to set
        """
        if len(vr) != len(value):
            raise ValueError("Number of value references must match number of value arrays")

        vr = (fmi2ValueReference * len(vr))(*vr)

        # # 计算数组大小并创建 C 数组
        # array_sizes = (ctypes.c_size_t * len(values))(
        ...
        for i, val in enumerate(value):
            size = len(val)
            # print("size是：",size)
            value_array = (fmi2Real * size)(*val)

            size_ptr = (fmi2Integer * 1)(size)
            vr_ptr = (fmi2ValueReference * 1)(vr[i])

            # 将 value_array 转换为 LP_LP_c_double 实例
            value_array_ptr = POINTER(fmi2Real)(value_array)
            # print("value_array_ptr",value_array_ptr[0],value_array_ptr[1],value_array_ptr[2]) #98.0 7.0 10.0
            self.fmi2SetRealArrayEx(self.component, vr_ptr, 1, value_array_ptr , size_ptr)
